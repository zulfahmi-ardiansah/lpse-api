...
def fill_obj_if_exist(obj, key):
    if key in obj:
        ret = str(obj[key]).strip()
        if ret != "None" and len(ret.strip()) > 0:
            return ret
    return None
...
# Get List Non Tender
def nontender_daftar(base_url: str, limit: int, chrome_driver: WebDriver, order: str = "desc"):
    # Variable
    result = {}
    base_url = base_url + lpse_version
    limit = 10 if limit == 0 else limit
    ...
    try:
        # Get Home Page
        chrome_driver.get(base_url + "/nontender")
        ...
        for tender in tender_json["data"]:
            tender_dictionary = {
                "url_detail": base_url + "/nontender/" + strip_tags(tender[0]) + "/pengumumanpl",
                "url_jadwal": base_url + "/nontender/" + strip_tags(tender[0]) + "/jadwal",
                "kode": strip_tags(tender[0]),
                "judul": strip_tags(tender[1]).replace("Tender Ulang", "(Tender Ulang)"),
                "instansi": strip_tags(tender[2]),
                "proses": strip_tags(tender[5]),
...
# Get Non Tender Jadwal
def nontender_jadwal(url: str, chrome_driver: WebDriver):
    # Variable
    result = {}
...
# Get Non Tender Detail
def nontender_detail(url: str, chrome_driver: WebDriver):
    # Variable
    result = {}
...
def lpse_daftar():
    ...
    try:
        with open("static/lpse-list.json", "r") as data_file:
            url_result = data_file.read()
        url_result = json.loads(url_result)
