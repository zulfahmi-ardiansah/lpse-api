...
# Strip Tags
def strip_tags(ret):
    return re.sub(re.compile("<.*?>"), "", str(ret)).replace("[...]", "")
...
def tender_daftar(base_url: str, limit: int, chrome_driver: WebDriver, order: str = "desc"):
    ...
    try:
        ...
        for tender in tender_json["data"]:
            tender_dictionary = {
                "pengumuman": base_url + "/lelang/" + strip_tags(tender[0]) + "/pengumumanlelang",
                "jadwal": base_url + "/lelang/" + strip_tags(tender[0]) + "/jadwal",
...
# Get Tender Jadwal
def tender_jadwal(base_url: str, code: str, chrome_driver: WebDriver):
    # Variable
    result = {}
    base_url = base_url + lpse_version
    ...
    try:
        # Get Detail Page
        chrome_driver.get(base_url + "/lelang/" + code + "/jadwal")
        ...
        result = define_response(jadwal_array)
...
# Get Tender Detail
def tender_detail(base_url: str, code: str, chrome_driver: WebDriver):
    # Variable
    result = {}
    base_url = base_url + lpse_version
    ...
    try:
        # Get Detail Page
        chrome_driver.get(base_url + "/lelang/" + code + "/pengumumanlelang")
